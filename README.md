...
把需求直接跟在命令后面，一句话就行。你说的**时间范围**和**主题**，Claude 都会照着筛：

```
/vibe-story 把我今天在这个项目里的开发过程写成一篇开发日志
/vibe-story 写一篇 6 月 10 日的开发复盘
/vibe-story 总结最近一周我在做的支付功能，写成技术分享
/vibe-story 把上周三到周五调试登录 bug 的过程，写成一篇故事
```

你可以指定：

- **时间**：今天 / 昨天 / 某个具体日期 / 最近一周 / 一段日期范围——只读这段时间的会话
- **主题**：只写某件事（比如"支付功能""那个登录 bug"），Claude 会在写作时只挑相关的内容，自动略过无关的会话

也可以只敲 `/vibe-story`，Claude 会反问你想写哪个项目、什么范围，再开始。

接下来它会找出相关会话、还原时间线、和你确认，然后把文章写好交给你。
不满意随时让它换标题、调风格、重写。
